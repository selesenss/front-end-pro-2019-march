...
4) Реализовать функцию `copy(list)` по копированию массива.

5) Предусмотреть возможность передачи вторым аргументом функции. При копировании массива - функция применится к каждому элементу копируемого массива. `newL = copy(list, function(value){ return value*10; })`